...
<form method="POST" action="{{ url('users/update', $roles->id) }}">
    ...
    <div class="form-group">
        ...
        <input type="email" class="form-control" id="email" name="email" required value="{{ ($roles->email ?? '')}}">
    </div>
    <div class="form-group">
        <label for="password">Password</label>
        <input type="password" class="form-control" id="password" name="password" required value="{{($roles->password ?? '')}}">
    </div>
